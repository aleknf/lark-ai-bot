...
exports.routeToLarkCLI = routeToLarkCLI;
exports.handleCommonQuery = handleCommonQuery;
 ...
 */
async function routeToLarkCLI(userPrompt, chatHistory = []) {
    try {
...
Available lark-cli domains: ${lark_cli_1.LARK_DOMAINS.join(", ")}
...
`;
        const messages = (0, openrouter_1.buildMessages)(systemPrompt, chatHistory, userPrompt);
        const result = await (0, openrouter_1.chatCompletion)(messages, { temperature: 0.3, maxTokens: 1000 });
        // Parse AI response
        ...
        for (const cmd of parsed.commands) {
            try {
                const result = await (0, lark_cli_1.execLarkCommand)(cmd.domain, cmd.args, { as: cmd.as || "user" });
                results.push(result);
            }
            catch (error) {
                utils_1.logger.error({ err: error, cmd }, "Failed to execute lark-cli command");
...
Format these results into a clear, user-friendly response. Use bullet points, formatting, and emojis where appropriate. Be concise but informative.`;
        const formatMessages = (0, openrouter_1.buildMessages)("You are a helpful assistant that formats Lark data into user-friendly responses.", [], formatPrompt);
        const formatted = await (0, openrouter_1.chatCompletion)(formatMessages, { temperature: 0.5, maxTokens: 2000 });
        return {
            success: true,
            data: results,
        ...
        utils_1.logger.error({ err: error }, "AI Lark router failed");
        return {
            success: false,
 ...
 */
async function handleCommonQuery(query) {
    const lowerQuery = query.toLowerCase();
    // Calendar/Schedule queries
    if (/\b(agenda|schedule|jadwal|kalender|calendar|meeting|rapat)\b/i.test(query) &&
...
async function handleCalendarQuery(query) {
    try {
        const now = new Date();
        ...
        const formatDateTime = (d, hour) => `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}T${hour}:00:00${tzSign}${tzH}:${tzM}`;
        let start, end, label;
        ...
        const result = await (0, lark_cli_1.execLarkCommand)("calendar", [
            "+agenda",
        ...
        utils_1.logger.error({ err: error }, "Calendar query failed");
...
async function handleTaskQuery() {
    try {
        ...
        utils_1.logger.error({ err: error }, "Task query failed");
        return `❌ Failed to fetch tasks: ${error instanceof Error ? error.message : "Unknown error"}`;
    }
}
async function handleContactSearch(query) {
    try {
        const result = await (0, lark_cli_1.execLarkCommand)("contact", ["+search-user", "--query", query, "--as", "user"]);
        ...
        utils_1.logger.error({ err: error }, "Contact search failed");
        return `❌ Failed to search contacts: ${error instanceof Error ? error.message : "Unknown error"}`;
    }
}
//# sourceMappingURL=ai-lark-router.js.map